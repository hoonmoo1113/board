...
const getTurn=()=>J('board:turn',NEW_TURN());
const setTurn=t=>setS('board:turn',JSON.stringify(t));
...
// 참가자가 직접 선택 (PIN 불필요) — 현재 차례 참가자 이름으로 채워지고 다음 차례로
...
app.post('/api/turn', async (req,res)=>{
  try{ if(!await auth(req))return res.status(403).json({error:'pin'});
...
app.post('/api/unclaim', async (req,res)=>{
  try{ if(!await auth(req))return res.status(403).json({error:'pin'});
    const {boardId,cell}=req.body||{}; await db.execute({sql:`DELETE FROM board_claims WHERE board_id=? AND cell=?`,args:[boardId,cell]}); res.json({ok:true});
  }catch(e){ res.status(500).json({error:String(e)}); }
});
