...
app.post('/api/undo', async (req,res)=>{ try{ if(!await auth(req))return res.status(403).json({error:'pin'});
  const players=await getPlayers(); const last=await db.execute(`SELECT board_id,cell,player_id FROM board_claims ORDER BY ts DESC LIMIT 1`);
...
app.post('/api/unclaim', async (req,res)=>{ try{ if(!await auth(req))return res.status(403).json({error:'pin'});
  const {boardId,cell}=req.body||{}; await db.execute({sql:`DELETE FROM board_claims WHERE board_id=? AND cell=?`,args:[boardId,cell]}); res.json({ok:true}); }catch(e){res.status(500).json({error:String(e)});}});
...
app.post('/api/board', async (req,res)=>{ try{ if(!await auth(req))return res.status(403).json({error:'pin'});
  ...
  else if(action==='delete'){ if(boards.length<=1) return res.status(400).json({error:'last'}); boards.splice(idx,1); await db.execute({sql:`DELETE FROM board_claims WHERE board_id=?`,args:[boardId]}); }
...
app.post('/api/reset', async (req,res)=>{ try{ if(!await auth(req))return res.status(403).json({error:'pin'});
  await db.execute(`DELETE FROM board_claims`); let t=await getTurn(); t.started=false; t.curIdx=0; t.remaining=t.picksPerTurn||1; await setTurn(t);
